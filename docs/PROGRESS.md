...
- [x] 多钱包换汇记录（2026-08-27）—— AddEntry 加了"Move"模式，选两个钱包；
      ...
      **换汇多了一种填法（2026-08-29，Zachary 要求）**：原本只能"输入汇率，系统
      算出拿到多少"，但真的去柜台换钱时经常是"钱已经换好拿到手了，汇率是多少
      早就不记得"。加了个切换："I know the rate"（原本那种）/ "I know what I
      received"（改成直接填拿到的印尼币金额），选第二种时汇率反过来用两个金额
      推算、存进 `fx_rate` 只是为了留档跟给下一次"建议汇率"用，不影响这笔记录
      本身对不对。
...
- [~] Web Push 通知（2026-08-29）—— 代码全部写完，**但要 Zachary 手动做三步才能真的用**
      ...
      **架构（2026-08-29 改过一版，范围变广了）**：Zachary 明确要求"这本账里发生
      任何事都要推"——不只是记账，评论、加钱包、删记录都要通知。原本设计只盯着
      `entries` 表加了个 `notified_at` 字段，评论/加钱包压根不发生在这张表上，
      所以改成一张通用的 `activity_events` 表：记账的新增/编辑/删除/还原、加评论、
      加钱包，各自的触发器都往这张表里存一句预先拼好的人话摘要。每天固定时间
      （默认 UTC 13:00 = 大马时间 21:00）另外还有一条当日财务汇总（这个还是按
      钱包算金额，没跟着扩大范围），钱包当天没动静就不推。
      **通知从"攒一分钟"改成几乎即时（2026-08-29）**：一开始设计成 `pg_cron`
      每分钟跑一次、等 10 秒让连续几个动作落定后合并成一条推送，理由是"一顿饭
      连记 4 笔，每笔都推的话家人第三天就把通知关了"。但 Zachary 反馈等 1-2
      分钟才收到通知太没效率，问清楚之后决定接受"可能连响好几声"这个代价，
      换成几乎即时：改成 `activity_events` 表自己的 insert 触发器直接呼叫
      `pg_net`，写进去的当下就送到 Edge Function `send-push`，不再排队等 cron。
      同一个人自己做的这个动作不会推给他自己，换了别人做，会立刻收到。原本的
      ...
      **通知开关改成一个铃铛图标**（2026-08-29，Zachary 要求）：点一下切换开/关，
      ...
      网站的通知权限本来就还没决定过，浏览器会跳出系统授权框，同意了就直接开了；
      如果已经同意过，就完全无声无息直接订阅成功。只在设备上试这一次，不会每次
      打开都烦。真被系统层面拒绝过（denied）的话，铃铛会关闭且点不动，提示去手机
      浏览器设置里手动开。
      **iOS 限制**：iOS 只有「已经加到主屏幕」的 PWA 才能用 Push API——在 Safari
      分页里 `Notification`/`PushManager` 这些接口根本不存在，App 会侦测到这个
      情况，把铃铛换成提示"先加到主屏幕"，不会让人点了没反应。
      Android 没有这个限制，Chrome 分页也能订阅推送。
      **顺手修了一个坑**：不管是原来只盯 entries 的版本，还是现在的 activity_events
      版本，"标记为已通知"这个内部更新如果照旧走 `log_entry_change()` 触发器，
      会在每笔账的历史记录里插入一条谁都没改过内容、`changed_by` 是空的"update"，
      家人点开会看到莫名其妙的改动记录——改了触发器，只有 `notified_at` 之外的
      字段真的变了才记历史。
...
- [x] 结算表（2026-08-29）—— 公式跟 CLAUDE.md 一致：该退你多少 = 你交了多少 − 总消费 ÷
      人数，单元测试直接照抄铁律里那个 5 人不均出资的例子（A 该退 1,100，其他人各退
      100），连同"出资相同时退化成平分剩余""算进还没加入的挂名出资人""排除
      换汇/转账和已软删除的记录"几种情况一起测了，共 4 条。
      **两个设计问题先问过 Zachary 才动手**：① 多钱包货币不一样要不要合并成一个数字——
      选了「每个钱包分开显示，不合并」，所以结算直接不用碰汇率换算，干净很多；
      ② 确认结算后账本要不要锁住——选了「不锁，只存一份快照」，新建了一张
      `settlements` 表存当时算出来的结果（谁交了多少/该承担多少/该退多少），
      不生成任何抵消记账的 entries，也不影响 admin 之后继续记账/改动的权限。
      Home 页余额下面加了一个「Settlement」入口，所有人都能看（透明），admin 才能
      按「存成快照」和删快照。
- [ ] 导出 CSV
- [x] 行程列表长按置顶/删除（2026-08-29，Zachary 临时加的，不在原本 Day 3 清单
      里）—— 一个账号能加入很多本账（每本账是不同的 group），长按某本账的框框
      会跳出菜单：Pin to top（置顶）/ Unpin、Delete trip（仅限该行程的 owner）。
      置顶存在 `members` 表自己的 `pinned_at` 字段——**这是"你自己"的偏好，不是
      整本账共用的排序**：你把某本账置顶，不会影响家人看到的顺序，他们各自置顶
      也不会互相干扰。改这个字段用了一个专门的 `set_trip_pinned()` 函数而不是
      开一条通用的"member 能改自己那一行"policy——如果直接开放通用 update
      policy，等于顺便让 member 能把自己的 `role` 改成 owner，是个权限漏洞；
      函数把能动的范围锁死在这一个字段。删除整本账用的权限（`trips_owner_delete`）
      其实步骤 5 建表的时候就有了，只是从来没做对应的前端功能，这次一起补上——
      删除是真删（级联清掉这本账所有的钱包/记账/评论/邀请链接/结算快照），
      要求手动打出行程名字才能确认，防止手滑。**已知的小缺口**：Storage 里的
      收据照片文件不会跟着自动清掉（级联删除只对数据库表有效，对 Storage
      没用）——删掉的行程通常不会再有人回头看那些照片，先不处理，之后介意
      存储空间再回来补清理逻辑。
