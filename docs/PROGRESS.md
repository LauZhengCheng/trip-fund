...
- [~] Web Push 通知（2026-08-29）—— 代码全部写完，**但要 Zachary 手动做三步才能真的用**
      ...
      **架构（2026-08-29 改过一版，范围变广了）**：Zachary 明确要求"这本账里发生
      ...
      加钱包，各自的触发器都往这张表里存一句预先拼好的人话摘要。`pg_cron` 每分钟
      跑一次，把同一本账还没通知过的事件（等 10 秒让连续几个动作落定）合并成一条
      推送——事件数超过 1 条时，推送内容显示第一条 + "还有 N 条"；同一个人自己
      连续做的这一批不会推给他自己，换了别人一起动手，大家都收到。`pg_net` 把
      算好的内容发给 Edge Function `send-push`，真正的 VAPID 签名/加密/发送在那边
      用 `web-push` 这个 npm 包做。每天固定时间（默认 UTC 13:00 = 大马时间 21:00）
      另外还有一条当日财务汇总（这个还是按钱包算金额，没跟着扩大范围），钱包当天
      没动静就不推。
